...
Today's `redirects.json` has 56 rules and zero chains — every target is terminal. The plugin handles multi-hop by design so future authoring can chain without extra wiring.
...
"Backfill" means retroactively adding entries that should have existed when older moves happened. The initial strict-mode pass backfilled 19 such misses — every one from a page moved or renamed in 7.2 without an accompanying `redirects.json` update, which the verifier caught the moment strict mode was turned on.
...
| `compareVersions` source | Plugin-local: `lib/compare-versions.ts`                                 | Inline in `handle_redirects.js` (CloudFront Functions can't resolve imports at edge time)   |
| Drift guard              | Parity test reads `handle_redirects.js` as text, regex-extracts the function, asserts behavioural parity against the plugin's TS copy on a case table | Same test |
