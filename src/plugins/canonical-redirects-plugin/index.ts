...
export interface CanonicalRedirectsPluginOptions {
    ...
    /**
     * When true, the build fails if the verifier reports any issues.
     * Defaults to `process.env.DOCUSAURUS_STRICT_CANONICALS === "true"` — a
     * dedicated gate separate from Docusaurus's own DOCUSAURUS_STRICT, because
     * canonical correctness depends on the content of redirects.json (not just
     * the plugin's correctness). Populate redirects for every moved/renamed
     * page first, then flip this flag in CI.
     */
        ...
        async postBuild(props) {
            ...
            walk(outDir, (filePath) => {
                ...
                if (result.newCanonical) {
                    ...
                    // A versioned page with no <link rel="canonical"> in the emitted
                    // HTML. Docusaurus stamps one on every page today — if this fires
                    // it usually means Docusaurus changed its HTML emission and the
                    // rewriter's regex needs updating. Count + warn so it surfaces in
                    // the build log without failing the build.
